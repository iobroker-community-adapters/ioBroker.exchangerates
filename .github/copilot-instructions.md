...
**Adapter Name:** exchangerates  
**Version:** 0.1.0  
**Primary Function:** Retrieve and monitor exchange rates from multiple financial data sources  
**Key APIs/Services:**
- **CBR (Central Bank of Russia)**: XML-based API for Russian ruble exchange rates
- **ECB (European Central Bank)**: XML-based API for Euro exchange rates  
- **Poloniex**: JSON REST API for cryptocurrency exchange rates

**Configuration Requirements:**
- Source selection (CBR, ECB, or Poloniex)
- Currency pair filtering and selection
- Update intervals and polling frequency
- Authentication handling for Poloniex API

**Data Processing:**
- XML parsing for CBR and ECB feeds using xml2js library
- JSON parsing for Poloniex API responses
- Currency code mapping and conversion
- State management for exchange rate data and metadata

**Unique Requirements:**
- Handle different data formats (XML vs JSON) from various financial institutions
- Implement proper error handling for network timeouts and API limitations
- Support multiple currencies and exchange rate sources simultaneously
- Manage rate limiting and API quotas for external services
...
### Exchange Rates Adapter Specific Patterns

**API Integration Patterns:**
- Use `xml2js` library for parsing CBR and ECB XML responses
- Implement timeout handling for all external API calls (default: 30 seconds)
- Use async/await pattern for API requests with proper error wrapping
- Cache API responses temporarily to avoid unnecessary calls during the same polling cycle

**Data Structure Conventions:**
- Store exchange rates as numbers with appropriate decimal precision (typically 4-8 decimal places)
- Use ISO currency codes (USD, EUR, RUB, etc.) for state naming
- Create channel structure: `source.currencyPair.current` and `source.currencyPair.percentChange`
- Include metadata states: `source.Date` for last update timestamp

**Error Handling Specifics:**
- Log API connection failures as warnings, not errors (to avoid spam)
- Implement exponential backoff for failed API requests
- Validate currency data before setState() calls
- Handle API-specific error codes (HTTP 429 for rate limiting, etc.)

**Configuration Management:**
- Support dynamic source switching without adapter restart
- Validate currency pair availability for selected source
- Implement configuration migration for version updates
- Use native configuration object for API credentials (encrypted for Poloniex)
